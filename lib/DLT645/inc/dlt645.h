...
{
	uint8_t MessageToSend[1024];
	uint16_t DataLenToSend;
	uint8_t MessageToRecv[1024];
	uint16_t DataLenToRecv;
    uint8_t addr[6];    //从机地址
    uint8_t debug;      //调试标志
    int (*write)(struct dlt645 *ctx);     //底层写函数
    int (*read) (struct dlt645 *ctx, uint16_t len);     //底层读函数
