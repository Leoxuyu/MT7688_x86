...
{
    int msg_len = ctx->read(ctx,len);

    if (protocal == DLT645_1997)
    {
        return dlt645_1997_recv_check(ctx->MessageToRecv, msg_len, ctx->addr, code);
    }
    else if (protocal == DLT645_2007)
    {
        return dlt645_2007_recv_check(ctx->MessageToRecv, msg_len, ctx->addr, code);
    }
    else
    {
        return -1;
...
int dlt645_send_msg(dlt645_t *ctx, uint8_t *msg, int len)
{
    ...
    memcpy(ctx->MessageToSend,msg,len);
    ctx->DataLenToSend=len;
